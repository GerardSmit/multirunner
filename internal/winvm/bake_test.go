...
func TestAutounattendFiles(t *testing.T) {
	files, err := AutounattendFiles("9.9.9", "P@ss123")
	if err != nil {
		t.Fatal(err)
	}
	if !strings.Contains(files["install-golden.ps1"], "9.9.9") {
		t.Error("runner version not substituted")
	}
	if strings.Contains(files["install-golden.ps1"], "__RUNNER_VERSION__") {
